...
// 获取每个
    ...
    const items = await Array.from($('#ctl00_ContentPlaceHolder1_gv100 .dot_gray'))
...
//   console.log(data);
  ...
  writerStream = fs.createWriteStream(directory+"/church100.json");
  ...
  writerStream.end();
    ...
    // fs.writeFile(directory+"/church100.json", data, 'utf8', function (err) {
    //     if (err) {
    ...
    fs.writeFile(directory+'/church100.csv', csv, function(err) {
        if (err) {
            return console.log(err);
        }
